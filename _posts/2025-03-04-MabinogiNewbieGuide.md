...
- 스킬 수련은 다음 중 3가지를 포함하여 항상 총 8배 횟수로 수련하는 것을 기본으로 합니다.
    ...
    - 수련 포션 ('종합 스킬 수련 포션', '종합 전투 스킬 수련 포션' 등)
...
- 던전 가이드에서 특정 던전의 첫 클리어 보상으로 얻게 되는 제 1 ~ 4 지원품 상자는 아르카나 활성화 이후에 개봉합니다.(망각의 비약 30일 기간제)
...
- 조건이 충족 되면 대부분의 퀘스트는 자동으로 부엉이를 통해 날아옵니다. 만약 퀘스트가 오지 않는다면, 각 마을마다 있는 퀘스트 수주 게시판을 통해 시작할 수 있습니다.
    ...
    **퀘스트 1**
    ...
    -  [가이드] 알반 기사단 벨테인 특별조 → 견습 기사단 1명(카나 또는 로간) 영입 → 호감도 레벨 5 달성(투아림 부스트 선행)
...
- 이제부터는 도움을 받지 않고는 진행불가한 부분이 일부 존재합니다.(레드 드래곤, 몽환의 라비 등) 게임 내에서 다른 분들에게 도움을 요청하세요.
...
- 그동안 진행 하지 않았거나 마무리 하지 않은 컨텐츠(스킬, 메인스트림, 그랜드 마스터, 퀘스트 등)는 중간 또는 이후에 여유가 생긴다면 자율적으로 진행하면 됩니다.
    ...
    **퀘스트 2**
    ...
    - [콘텐츠] 검푸른 잠이 불어오는 겨울(글렌 베르나 입장 해금. 천옷 만들기, 블랙 스미스 최소 9랭크 이상 필요)
